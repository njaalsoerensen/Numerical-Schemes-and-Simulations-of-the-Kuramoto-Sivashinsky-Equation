...
def solve_history(nu, N, dt, T, u_0, L, save_name, nest_folder=False):
    ...
    Bank.plot_solve_history('Eq_ETD_RK4', title=f'{solve_method} solution, '+r'$\nu$ = '+f'{nu} {state}', lab=0, step_speed=200, N_simul=1000, ani=False, save=True)
...
def error_relationship(eq_str, u_0, L, N, nu, T, freqs, nums, solve_methods='ETD_RK4', nest_folder=False):
    colors = {'ETD2': 'blue','ETD_RK2':'orange', 'ETD_RK3':'green', 'ETD_RK4':'red', 'ETD_RK4_CM':'purple', 'Audun_solver':'cyan'}
    lines = ['solid', (0, (5, 4)), (0, (1, 2)), (0, (5, 3))]
    
    # Find the state given the value
    state = determine_state_type(nu)
    
    # Plor theese in log log plots
    if not nest_folder:
            nest_folder = f'{state}_{nu}'
    else:
        nest_folder = f'{nest_folder}/{state}_{nu}'
    folder = f'DATA/ERROR_RELATIONSHIP/{nest_folder}'
    os.makedirs(folder, exist_ok=True)
    ...
    for i in range(len(solve_methods)):
        plt.plot(freqs[1:], full_errors[i][1:], label=solve_methods[i], color=colors[solve_methods[i]], linestyle=lines[i])
    ...
    plt.ylabel('Total error')
    for i in range(len(solve_methods)):
        plt.plot(nums[1:], full_errors[i][1:], color=colors[solve_methods[i]], label=solve_methods[i], linestyle=lines[i])
    ...
    for i in range(len(solve_methods)):    
        ...
        plt.scatter(nums, full_times[i], color=colors[solve_methods[i]], alpha=0.5)
    plt.xscale('log')
    plt.yscale('log')
    plt.legend(loc=2)
...
def run_full_ETD(nu, N, dt, T, u_0, L=2*np.pi, plot=False, step_speed=1, save_name='TEMP', eq_str=False, nest_folder=False, mode='', start_mode=0, include_modes=False, in_time=0):
    ...
    for method_to_run in methods_to_run:
        ...
        if mode == 'FT_energy' or mode == 'Linear_growth': # Define important solutions as FT domai energyn, plots energy of modes
            ...
            if mode == 'Linear_growth':
                ...
                X_y = np.array([np.log(modes_energy[i][num]/modes_energy[i][0])/(2*num*dt)  for i in range(len(modes_energy))])
                # Note dividing by 2 here to go from energy of mode to value of mode
                ...
                os.makedirs(folder, exist_ok=True)
                name = f'Linear_growth_rate_{nu}_N_{N}_freq_{int(1/dt)}_T_{T}_inTime_{in_time}_modes_{include_modes}_num_{num}.png'
                path = os.path.join(folder, name)    
                plt.savefig(fname=path)
                plt.show()  
    ...
    def animate(t): # Returns time domain solution
        ...
        if mode=='Confirm_behaviour':
            ax.set_title(r'Solver methods $\nu$ = ' + f'{round(nu, 4)} {state}, Eq: {eq_str}, t = {round(t*step_speed*dt, 2)}')
        # if mode=='FT_energy':
...
def run_same_states(N, dt, T, u_0, L, eq_str, state_type, nu_values, num=5, mode='', step_speed=1, plot=False, nest_folder=False,start_mode=0, include_modes=20, in_time=0):
    # Computes for all same states
    print(f'\n{state_type}')
    u_0 = eval(f"lambda x: {eq_str}") 
    state_type_short =  states_short[state_type]
    ...
    for i in range(len(nu_values)):
        ...
        for nu in nu_linspace:
            if mode == 'Confirm_behaviour':                
                run_full_ETD(nu, N, dt, T, u_0, L=L, plot=plot, step_speed=step_speed, save_name='ETD_methods', eq_str=eq_str, nest_folder='Confirm_behaviour/'+state_type_short+f'_{i}', mode=mode)
                
            if mode == 'FT_energy':
                run_full_ETD(nu, N, dt, T, u_0, L=L, plot=plot, step_speed=step_speed, save_name='FT_energy', eq_str=eq_str, nest_folder='FT_energy/'+state_type_short+f'_{i}', mode=mode, include_modes=include_modes)
                
            if mode == 'Linear_growth':    
                run_full_ETD(nu, N, dt, T, u_0, L=L, plot=plot, step_speed=step_speed, save_name='Linear_growth', eq_str=eq_str, nest_folder=f'LINEAR_GROWTH/{include_modes}_MODES/'+state_type_short+f'_{i}', mode=mode, start_mode=start_mode, include_modes=include_modes, in_time=in_time)
            
            if mode == 'Confirm_energy':
                eq_str = eq_str.replace('.', '_')
                energy_confirmation(nu, N, dt, T, u_0, L, in_time=5, plot=plot, nest_folder=eq_str+'/'+state_type_short+f'_{i}')    
            
            if mode == 'Solve_hostory':
                eq_str = eq_str.replace('.', '_')
                solve_history(nu, N, dt, T, u_0, L, save_name='Solve_history', nest_folder=eq_str+'/'+state_type_short+f'_{i}')

def determine_state_type(nu):
    try:
        for key, values in states.items():
            if type(values[0]) == int:
                value = values
                if nu > value[0] and nu < value[1]:
                    state = key
            else:
                for value in values:
                    if nu > value[0] and nu < value[1]:
                        state = states_short[key]
        return state
    except UnboundLocalError: return '(?)' # Generally state is not defined, common is in between 2 defined states, 
...
states_short = {'Periodic_attractors_complete_period_doubling': '(PA2)',
              'Constant_states': '(CS)',
              'Fully_modeal_steady_attractors': '(S)',
              'Bimodal_steady_attractors': '(S2)',
              'Trimodeal_steady_attractors': '(S3)',
              'Tetramodal_steady_attractors': '(S4)',
              'Periodic_attractors': '(PA)', 
              'Chaotic_oscillations': '(C)'} 
